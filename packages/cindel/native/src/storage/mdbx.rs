...
impl MdbxStorage {
    ...
    fn next_id_from_storage(&self, collection: &str) -> Result<u64, String> {
        self.with_read_transaction(|transaction, tables| {
            ids::next_id_from_storage(transaction, &tables, collection)
        })
    }

    fn persist_next_id_at_least(&self, collection: &str, next_id: u64) -> Result<(), String> {
        self.with_write_transaction(|transaction, tables| {
            ids::set_persisted_next_id_at_least(transaction, &tables, collection, next_id)
        })
    }
...
impl StorageEngine for MdbxStorage {
    ...
    fn put_indexed(
        ...
        self.with_write_transaction(|transaction, tables| {
            ...
            ids::set_persisted_next_id_at_least(
                transaction,
                &tables,
                collection,
                id.saturating_add(1),
